...
class VolunteerTrackingApplicationTests {
	...
	void e_operationsFromOtherUser() {
		try {
			...
			op = op.replace(
					("Hi" + name2 + "!LogoutMyEventsEventsVolunteeredForUpcomingNGOEventsCreateNGOEventMyEvents")
							.toLowerCase(),
					"");
			...
			op = "Hi" + name2
					+ "!LogoutMyEventsEventsVolunteeredForUpcomingNGOEventsCreateNGOEventEventsVolunteeredFor";
			...
			assert (op.contains(("Hi" + name2
					+ "!LogoutMyEventsEventsVolunteeredForUpcomingNGOEventsCreateNGOEventUpcomingNGOEvents")
					.toLowerCase()));
			op = op.replace(("Hi" + name2
					+ "!LogoutMyEventsEventsVolunteeredForUpcomingNGOEventsCreateNGOEventUpcomingNGOEvents")
					.toLowerCase(), "");
			op = op.replace(
					(event11 + "Location:location11Date:2025-10-10Noofvolunteers:0VolunteerForEvent").toLowerCase(),
					"");
			assert (op.contains(
					(event12 + "Location:location12Date:2025-10-10Noofvolunteers:0VolunteerForEvent").toLowerCase()));
			op = op.replace(
					(event12 + "Location:location12Date:2025-10-10Noofvolunteers:0VolunteerForEvent").toLowerCase(),
					"");
			...
			assert (page.asText().replaceAll("\n", "").replaceAll(" ", "").toLowerCase()
					.contains("Noofvolunteers:1".toLowerCase()));
			page = webClient.getPage("http://localhost:" + port + "/app/home");
			page = ((HtmlElement) page.getByXPath("//*[text()='Events Volunteered For']").get(0)).click();
			op = page.asText().replaceAll("\n", "").replaceAll(" ", "").toLowerCase();
			op = op.replace(("Hi" + name2
			...
			assert (op.contentEquals((event11 + "location:location11").toLowerCase())
					|| op.contentEquals((event12 + "location:location12").toLowerCase()));
			((HtmlElement) page.getByXPath("//*[text()='Logout']").get(0)).click();
		} catch (Exception e) {
			e.printStackTrace();
			assert (false);
	...
	void f_finalCheckFromFirstUser() {
		try {
			...
			inps.get(1).setAttribute("value", pass1);
			page = ((HtmlElement) page.getByXPath("//*[text()='Login']").get(0)).click();
			String output = ("Hi" + name1
					+ "!LogoutMyEventsEventsVolunteeredForUpcomingNGOEventsCreateNGOEventMyEvents").toLowerCase();
			String op = page.asText().replaceAll("\n", "").replaceAll(" ", "").toLowerCase();
